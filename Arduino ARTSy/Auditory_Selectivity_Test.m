%% Auditory Selectivity Test 07/16/2019 by Yow-Tyng (Tim) Yehs
...
tones = dir('C:\Users\labadmin\Desktop\Arduino ARTsy\Testing_songs\Tones');
...
for i=1:(length(tones)-2)
    path = fullfile('C:\Users\labadmin\Desktop\Arduino ARTsy\Testing_songs\Tones', tones(i+2).name);
    info = audioinfo(path);
...
playList = cell(size(tones_cell,1)/7, 1);  % empty cell for storing audio signals of all blocks

for i=1:(length(tones_cell)/7)
    block = [tones_cell((i*7-6):(i*7),:);sham_cell];
...
final_playlist = cell2table(final_playlist);
filename = fullfile('C:\Users\labadmin\Desktop\Arduino ARTsy\Data\Auditory_sensitivity_data',['AudioTest_booth', num2str(booth),'_', birdID, '_', sex, '_', num2str(current_time(1))...
    , '_',num2str(current_time(2)),'_',num2str(current_time(3)), '.xlsx']);
writetable(final_playlist, filename);